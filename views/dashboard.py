"""
Dashboard View - Kaspa Analytics Pro
Main dashboard content for authenticated and public users
"""
    ...
    with pricing_cols[0]:
        with st.container():
            ...
            if st.button("🚀 Get Started Free", key="pricing_free", use_container_width=True, type="primary"):
                st.switch_page("views/authentication.py")
    ...
    with action_cols[3]:
        if subscription in ['premium', 'pro']:
            if st.button("📋 Data Export", key="dash_export", use_container_width=True):
                st.switch_page("views/data_export.py")
        else:
